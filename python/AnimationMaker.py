...
nuke.menu('Animation').addCommand( 'Animation Maker...', 'reload(AnimationMaker);AnimationMaker.showWindow()','',icon='ParticleBounce.png')
...
class animationWindow(QtGui.QWidget):
    ...
    def __init__(self, knobName, knobIndex = -1, parent=None):
        ...
        if self.knobIndex > -1:
            ...
            self.originalValues = self.toScriptMultiple(self.originalValue)
        else:
        ...
        self.copyrightLabel.setText('<p style="font-family:arial;color:gray;font-size:10px;">AnimationMaker 1.2 - David Emeny 2016</p>')
        ...
        self.presetLabel = QtGui.QLabel(self)
        ...
        #PRESET BUTTONS
        self.presetReloadButton = QtGui.QPushButton('reload preset', self)
        ...
        self.presetUpdateButton = QtGui.QPushButton('save preset', self)
        ...
        self.presetDeleteButton = QtGui.QPushButton('delete preset', self)
    ...
    def toScriptMultiple(self,val_string):
...
if k.name() == ("a_basechoice_" + knob_name):
    ...
    if k.value() == "Add":
        if knob_index > -1:
            n[orig_knob_name].setExpression(n['a_exprTextAdd_' + knob_name].value(),knob_index)
        else:
        ...
        if knob_index > -1:
            n[orig_knob_name].setExpression(n['a_exprTextMult_' + knob_name].value(),knob_index)
        else:
    ...
    if n['a_basechoice_' + knob_name].value() == "Equilibrium":
        ...
        ''' %(self.knobIndex,self.theKnobName,self.origKnobName,orig_value_string,self.originalValue)
...
class lineGraphic(QtGui.QGraphicsLineItem):
    ...
    def __init__(self, parent, x1,y1,x2,y2,colour):
        ...
        drop = QtGui.QGraphicsDropShadowEffect(parent)
